...
### 4.0.2 决策链路与工程化 P0 治理计划 (Based on Audit)

> 来源：`docs/17-agent-decision-audit.md` (Agent 决策链路审计)

我们对 Agent 核心决策链路进行了深度审计，识别出以下严重影响可追溯性与稳定性的 P0 问题，需立即治理。

#### P0-1 Trace ID 稳定性治理
- **问题**：`hash((session_id, user_text))` 依赖进程随机种子，跨进程不一致，导致日志/Bug报告无法归因。
- **评分现状**：1/5 (不可用)
- **目标方案**：`uuid4` (5/5 工业级)
- **状态**：⏳ 待排期 (Next)

#### P0-2 控制协议结构化
- **问题**：`STEP_DONE` 字符串匹配易误触，协议脆弱。
- **评分现状**：2/5 (脆弱)
- **目标方案**：Strict JSON Envelope `{"control": "step_done"}` (4.5/5 健壮)
- **状态**：⏳ 待排期

#### P0-3 局部重规划 (Plan Patching)
- **问题**：全量重写 Plan 成本高且丢失上下文。
- **评分现状**：2/5 (昂贵)
- **目标方案**：`PlanPatch` 增量修补策略 (4.5/5 高效)
- **状态**：⏳ 待排期
...
- `docs/17-agent-decision-audit.md`：Agent 决策链路审计（业界对标），包含决策链路 E2E、P0/P1/P2 问题清单、根因与模块级开发计划/验收标准。
